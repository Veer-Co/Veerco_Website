...
function detect_old_ie() {
    ...
    function a(aw, aF) {
        ...
        var N;
        var aC = {};
        var ac, aj, Z, ai, ab, ah, al, S, aE, y, an;
        var ax, o, Q, U, T, aa, p = new Array();
        var G = new Array(),
            aB = 0,
            z = 0;
        var J, R, l, k;
        var ap, aA, av, at, P, n, aK, aI, az, ay, X, V, ao, ak = 0;
        var h, ag;
        var aJ, C = 0,
            A = 0,
            af = 0,
        ...
        function au() {
            ...
            var v = (window.pageXOffset || aL.scrollLeft) - (aL.clientLeft || 0);
            var u = (window.pageYOffset || aL.scrollTop) - (aL.clientTop || 0);
            return {
                left: v,
                top: u
            }
        ...
        function aD() {
            ...
            if (F.options.zoomWidth == "auto") {
                ...
                Z = g / m * F.options.zoomWidth
            }
            if (F.options.zoomHeight == "auto") {
                ai = i
            } else {
                ai = g / q * F.options.zoomHeight
            }
            if (F.options.position.substr(0, 1) == "#") {
                aC = b(F.options.position)
            } else {
                aC.length = 0
            }
            if (aC.length != 0) {
        ...
        this.xscroll = function(v) {
            v.preventDefault();
            if (v.xscale) {
                var u = v.pageX || v.originalEvent.pageX;
                var aM = v.pageY || v.originalEvent.pageY;
                ak = v.xscale;
                M(u, aM)
            } else {
                var aL = -v.originalEvent.detail || v.originalEvent.wheelDelta || v.xdelta;
                var u = v.pageX || v.originalEvent.pageX;
                var aM = v.pageY || v.originalEvent.pageY;
                if (E) {
                    u = Y;
        ...
        function x() {
            if (F.options.lensShape == "circle" && F.options.position == "lens") {
                ap = aA = Math.max(ap, aA);
                var u = (ap + Math.max(n, P) * 2) / 2;
                l.css({
                    "-moz-border-radius": u,
            ...
            if (F.options.position == "lens") {
                ...
                if (F.options.bg) {
                    l.css({
                        ...
                        "background-position": (-(u - al) * az + (ap / 2)) + "px " + (-(aM - ah) * ay + (aA / 2)) + "px"
                    });
                    if (aL && v) {
                        l.css({
                            "background-size": aL + "px " + v + "px"
                        })
                    }
                }
            } else {
        ...
        function c() {
            ...
            aO += (A - aO) / F.options.smoothLensMove;
            v += (C - v) / F.options.smoothZoomMove;
            aL += (A - aL) / F.options.smoothZoomMove;
            aN += (af - aN) / F.options.smoothScale;
            aM += (ad - aM) / F.options.smoothScale;
            az = aN / ac;
            ay = aM / aj;
            ap = Z / az;
            aA = ai / ay;
        ...
        function O(u, aL) {
            ...
            ab.top = Math.round(ab.top);
            ab.left = Math.round(ab.left);
            ...
            if (aC.length == 0 && F.options.position != "inside" && F.options.position != "fullscreen") {
                ...
                if (F.options.zoomWidth == "auto") {
                    ...
                    Z = ac / m * F.options.zoomWidth
                }
                if (F.options.zoomHeight == "auto") {
                    ai = aj
                } else {
                    ai = ac / q * F.options.zoomHeight
                }
                S += F.options.Yoffset;
                aE += F.options.Xoffset;
                Q.css({
                    width: Z + "px",
                    height: ai + "px",
                    top: S,
                ...
                if (F.options.position == "inside" || F.options.position == "fullscreen") {
                    ...
                    S += (aC.outerHeight() - ai - Q.outerHeight()) / 2;
                    aE += (aC.outerWidth() - Z - Q.outerWidth()) / 2;
                    Q.css({
                        width: Z + "px",
                        height: ai + "px",
                        top: S,
            ...
            if (F.options.title && j != "") {
                if (F.options.position == "inside" || F.options.position == "lens" || F.options.position == "fullscreen") {
                    ...
                    y = S + (Q.outerHeight() - ai) / 2;
                    an = aE + (Q.outerWidth() - Z) / 2;
                    N.append(w)
                }
                w.css({
                    width: Z + "px",
            ...
            if (F.options.tint && (F.options.position != "inside" && F.options.position != "fullscreen")) {
                ...
                if (E) {
                    o.css({
                        "background-image": "url(" + aw.attr("src") + ")",
        ...
        this.openzoom = function(u) {
            ...
            R.bind("load", function() {
                ...
                X = h / Z;
                V = ag / ai;
                n = (parseFloat(l.css("padding-top")) + parseFloat(l.css("padding-bottom")) + parseFloat(l.css("border-top")) + parseFloat(l.css("border-bottom"))) / 2;
                P = (parseFloat(l.css("padding-left")) + parseFloat(l.css("padding-right")) + parseFloat(l.css("border-left")) + parseFloat(l.css("border-right"))) / 2;
                M(u.pageX, u.pageY);
                if (t && !F.options.bg) {
                    aJ = true;
                    requestAnimFrame(c)
        ...
        this.movezoom = function(v) {
            ...
            if (u < 0 || u > ac || aL < 0 || aL > aj) {
                ax.trigger("mouseleave")
            }
            if (t && !F.options.bg) {
                C = v.pageX;
                A = v.pageY
        ...
        this.eventdefault = function() {
            F.eventopen = function(u) {
                u.bind("mouseenter", F.openzoom)
            };
            F.eventleave = function(u) {
                u.bind("mouseleave", F.closezoom)
            };
            F.eventmove = function(u) {
                u.bind("mousemove", F.movezoom)
            };
            F.eventscroll = function(u) {
                u.bind("mousewheel DOMMouseScroll", F.xscroll)
            };
            F.eventclick = function(u) {
                u.bind("click", function(v) {
                    aw.trigger("click")
                })
            }
        };
        this.eventunbind = function() {
            aw.unbind("mouseenter");
            F.eventopen = function(u) {};
            F.eventleave = function(u) {};
            F.eventmove = function(u) {};
            F.eventscroll = function(u) {};
        ...
        this.xappend = function(u) {
            ...
            G[aB] = v.attr("href");
            v.data("xindex", aB);
            if (aB == 0 && F.options.activeClass) {
                L = u;
                L.addClass(F.options.activeClass)
            }
            if (aB == 0 && F.options.title) {
                j = ae(u)
            }
            aB++;
            ...
            function aL(aN) {
                ...
                j = ae(u);
                if (u.attr("title")) {
                    aw.attr("title", u.attr("title"))
                }
                aw.attr("xoriginal", aO);
                aw.attr("src", aM)
            }
            if (F.options.hover) {
                v.bind("mouseenter", v, aL)
            }
            v.bind("click", v, aL)
        };
        this.init(aF)
